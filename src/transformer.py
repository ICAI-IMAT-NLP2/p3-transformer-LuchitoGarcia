...
class Transformer(nn.Module):
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token_indices = torch.multinomial(probs, num_samples=1)  # Shape: (batch_size, 1)
            # Map sampled indices to original token indices
            next_token = torch.gather(topk_indices, -1, next_token_indices)
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        # Pass the source input through the encoder
        attn_mask = kwargs.get('attn_mask', None)
        enc_output = self.encoder(src_input, attn_mask) if attn_mask is not None else self.encoder(src_input, mask=None)
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            expanded_tgt_input = tgt_input.expand(k, seq_len)   # Shape: (k, seq_len)
            next_tokens = topk_indices.view(k, 1)   # Shape: (k, 1)
            y_candidates = torch.cat([expanded_tgt_input, next_tokens], dim=1)  # Shape: (k, seq_len + 1)

            # Pass each candidate through the decoder
            dec_outputs_candidate = self.decoder(y_candidates, enc_output.expand(k, -1, -1))
            ...
            max_sim = torch.max(cos_sim, dim=-1)[0]  # Shape: (k,)

            # Compute scores
            P_LM_v = topk_probs.view(k)  # Shape: (k,)
            scores = alpha * P_LM_v - (1.0 - alpha) * max_sim  # Shape: (k,)

            # Select the candidate with the highest score
            best_idx = torch.argmax(scores) 
            best_token = topk_indices[0, best_idx].view(1, 1)  # Shape: (1, 1)
